...
### 2026-06-13 — Overlay render snapshot eval (closes the render blind-spot)
**Decision:** added a `GlyphTests` target + `GlyphTests/OverlaySnapshotTests.swift` — 6 tests that (a) **measure** the natural content size of the expanded overlay row and the nudge content (using the real tokens/styles via `NSHostingView.fittingSize`) and assert it fits the declared `OverlaySize`, (b) **smoke-render** the real `RecordingOverlayView`/`CallNudgeView` via `ImageRenderer` and assert non-blank, and (c) a **regression proof** that the paused row needs >248pt (old width clipped) and ≤336pt (current width contains). Wired into `_build_check.sh` as Gate-E (isolated `.build_dd`, hardened-runtime OFF for the test host only — the dev `build/` app stays hardened, TCC untouched).
**Why:** render-only defects (text wrap/clip/truncation) escaped to the user twice (v1.1.1 height, v1.1.2 wrap) because the floating NSPanel is never screenshot in QA — the loop-audit F4 register flagged it as a structural blind-spot. This gives the loop an automated render gate. **Limitation:** the containment checks use *mirror probes* of `expandedBar`/the nudge VStack (the views apply their fixed `.frame` internally, so the real view can't be measured for overflow) — keep the probes in sync with the source; the robust follow-up is to extract a shared, unframed content view that both the app and the test measure (supersedes the §4 "auto-size to content" item).
